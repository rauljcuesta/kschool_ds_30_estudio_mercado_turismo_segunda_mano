...
def obtain_ganvam_dataset(config, fraction = 1.0):
    import pandas as pd

    # columns = ['Q_query','year_Q','make','model','fuel','year_query','modelo','valor','cilindrada','puertas','potencia','startyear','endyear']
    columns = config['fields_ganvam_process']

    df = '';

    for ganvam_file in config['files_ganvam']:

        route_file = config['base_route'] + config['data_route'] +  config['data_ganvam_route'] + '/' + ganvam_file

        print(f'Processing {ganvam_file}')

        df_aux = pd.read_csv(route_file, compression='gzip', sep=config['csv_ganvam_separator'])
        df_aux = df_aux.sample(frac=fraction)
